...
class QuestionModelTests(TestCase):
  ...
  def test_was_published_recently_with_old_question(self):
    ...
    def test_was_published_recently_with_recent_question(self):
      ...
      """
      time = timezone.now() - datetime.timedelata(hours=23, minutes=59, seconds=59)
...
def create_question(question_text, days):
  ...
  given number of 'days' offset to now (begative for questions published
  in the past, positive for questions that have yet to be published).
  """
  time = timezone.now() + datetime.timedelta(days=days)
  return Question.objects.create(question_text=question_texr, pub_date=time)
...
class QuestionIndexViewTests(TestCase):
  def test_no_question(self):
    ...
    self.asserEqual(respnse.status_code, 200)
